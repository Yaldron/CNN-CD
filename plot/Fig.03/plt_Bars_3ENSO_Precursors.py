...
##============================================================================
data = pd.read_table("../../data/ENSO_Precursor/Normalized_idx_Precursors_in_JFM_1948-2016.txt",sep='\t')
year = data.loc[:,'Year'].values

idx_NPMM = data.loc[:,'NPMM'].values
idx_SPQ  = data.loc[:,'SPQ'].values
idx_TA   = data.loc[:,'TA-SSTA'].values
idx_TA   = idx_TA*-1.0
...
for i in range(success_year.shape[0]):
	NPMM[i] = idx_NPMM[success_year[i] - year[0]]
	SPQ[i]  = idx_SPQ[success_year[i] - year[0]]
	TAm1[i] = idx_TA[success_year[i] - year[0]]


for i in range(fail_year.shape[0]):
	NPMM_S[i] = idx_NPMM[fail_year[i] - year[0]]
	SPQ_S[i]  = idx_SPQ[fail_year[i] - year[0]]
	TAm1_S[i] = idx_TA[fail_year[i] - year[0]]
