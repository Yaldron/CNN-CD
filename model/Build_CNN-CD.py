...
##############################################################################
fsst1 = "../data/Train/ens43_CMIP6_tos_Ham-LR.nc"
fzos1 = "../data/Train/ens43_CMIP6_zos_Ham-LR.nc"
...
for i in range(3):
	for j in range(nens):
		...
		X[nlen * j:nlen * (j + 1), :, :, i + 3] = zosa1[j, 12 + monst - 1 - i:12 + monst - 1 - i + nlen * 12:12, :, :]
...
for j in range(nens):
	for ir in range(13):
		...
		Y[nlen * j:nlen * (j + 1), ir] = np.mean(np.mean(ssta1[j, 12 + monst + nstep:12 +monst + nstep + nlen * 12:12, id_lat, id_lon], axis=1), axis=1).reshape([nlen])

for ir in range(13):
	id_lon = (lon >= 130+ir*10) & (lon <= 150+ir*10)
	YT[:,ir] = np.mean(np.mean(ssta2[12 + monst + nstep:12 + monst + nstep +nvali * 12:12, id_lat, id_lon], axis=1), axis=1).reshape([nvali])
...
for k in range(nstep, nstep + 1):
	...
	for i in range(201):
	    ...
	    if i % record_step == 0:
			...
			flog = open("./log_" + train_obj + "_" + str(monst).zfill(2) +
				"_" + str(k + 1).zfill(2) + ".log", 'a+')
			print('RSME: ', sloss, file=flog)
			flog.close()
	...
	np.savetxt('fs_obs_'+str(monst).zfill(2) + 's_' + str(k + 1).zfill(2) + 'p_' + str(i).zfill(3) + 't.txt',YT[:],fmt="%0.5f")
	np.savetxt('fs_pre_'+str(monst).zfill(2) + 's_' + str(k + 1).zfill(2) + 'p_' + str(i).zfill(3) + 't.txt',yp[:],fmt="%0.5f")
