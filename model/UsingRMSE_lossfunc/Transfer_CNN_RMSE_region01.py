...
tmp = np.zeros((20 * 2, nvali), dtype=float)  #Save the test results
##############################################################
train_obj = 'CMIP6_ens43_OBS_trans'

for k in range(nstep, nstep + 1):
	print('the object mon is ', monst + k + 1)
	Y[:] = np.mean(np.mean(ssta1[12 + monst + k:12 + monst + k +
		ntrain * 12:12, id_lat, id_lon], axis=1), axis=1).reshape([ntrain, 1])
	YT[:] = np.mean(np.mean(ssta2[12 + monst + k:12 +
		monst + k + nvali * 12:12, id_lat, id_lon], axis=1), axis=1).reshape([nvali, 1])
	sess = tf.Session()

	dirpath = "./"+str(nstep + 1)+"p_model/"

	mo_path = "CMIP6_ens43_" + \
	...
	for i in range(21):
		...
		if i % record_step == 0:
			...
			flog = open("./log_" + train_obj +
				str(k + 1).zfill(2) + ".txt", 'a+')
	...
	DIR.to_csv(train_obj + str(monst).zfill(2) + 's_' +
