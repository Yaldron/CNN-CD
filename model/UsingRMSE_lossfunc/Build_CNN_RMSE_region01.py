...
#SSTA averaged of this domian is the predition of this CNN
id_lat = (lat >= -5) & (lat <= 5)
id_lon = (lon >= 130) & (lon <= 150)
...
for j in range(nens):
	Y[nlen * j:nlen * (j + 1), :] = np.mean(np.mean(ssta1[j, 12 + monst + nstep:12 + monst + nstep + nlen * 12:12, id_lat, id_lon], axis=1), axis=1).reshape([nlen, 1])
YT[:] = np.mean(np.mean(ssta2[12 + monst + nstep:12 + monst + nstep + nvali * 12:12, id_lat, id_lon], axis=1), axis=1).reshape([nvali, 1])
...
tmp = np.zeros((20 * 2, nvali), dtype=float)  #array save the test results
#############################################################
